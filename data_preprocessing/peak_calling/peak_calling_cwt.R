...
for (infile in list.files(folder)){
    ...
    ampTh <- 0.0001 #setting the minimum required relative amplitude threshold of peaks
    try({peakInfo <- peakDetectionCWT(MS.test,SNR.Th=SNR.Th,amp.Th=ampTh)
